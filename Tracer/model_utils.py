...
class Path(object):
	...
	def get_path_to(self, vertex, path=None, limit=6):
		# iteratively establish the path to vertex
		if path is None:
			path = []
		def follow(vertex):
...
def make_path_input(path, extension_vertex, segment_length, fname=None, green_points=None, blue_points=None, angle_outputs=None, angle_targets=None, action_outputs=None, action_targets=None, detect_output=None, detect_mode='normal', window_size=512):
	...
	#print("correct location:{}".format(extension_vertex.point))
	if not path.tile_data['rect'].add_tol(-int(window_size/2)).contains(extension_vertex.point):
		print(extension_vertex.point)
	...
	if fname is not None:
		...
		if angle_outputs is not None or angle_targets is not None:
			for i in range(window_size):
				for j in range(window_size):
					di = i - int(window_size/2)
					dj = j - int(window_size/2)
		...
		x[half_windowsize-3:half_windowsize+3, half_windowsize-3:half_windowsize+3, 2] = 1.0
		x[half_windowsize-3:half_windowsize+3, half_windowsize-3:half_windowsize+3, 0:2] = 0
	...
	new_shape=int(window_size/4)
	return input, detect_target.reshape(new_shape, new_shape, 1)
