...
class Model:
	...
	def __init__(self, input_channels):
		...
		# init input_channels=4

		self.is_training = tf.placeholder(tf.bool)
		self.inputs = tf.placeholder(tf.float32, [None, 256, 256, input_channels],name="input")
		...
		self.angle_loss = tf.reduce_mean(
			tf.reduce_mean(tf.square(self.angle_targets - self.angle_outputs), axis=1)*self.action_targets[:, 0])
